      ...
      <div className={`pattern-grid quiz-topic-grid ${view === "list" ? "quiz-topic-list" : ""}`}>
        {quizTopics.map((topic, index) => {
          ...
          return <article className={`pattern-card ${topic.accent}`} key={topic.id}>
            ...
            <div className="card-top"><span>{completed}/{questions.length} complete</span><progress className="mini-progress" value={completed} max={questions.length} aria-label={`${completed} of ${questions.length} questions complete`} />{completed > 0 && <button className="topic-reset" aria-label={`Reset ${topic.title} progress`} title={`Reset ${topic.title} progress`} onClick={() => onResetTopic(topic.id)}><RotateCcw size={13} /></button>}</div>
