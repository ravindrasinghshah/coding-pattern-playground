  ...
  return (
    <div className="app-shell">
      ...
      <Routes>
        ...
        <Route path="/practice" element={<Dashboard completedIds={knownCompletedIds} completedProblemIds={knownProblemIds} onOpen={(id) => navigate(`/practice/${id}`)} onReset={resetProgress} />} />
        ...
        <Route path="/practice/:patternId/templates/:templateId" element={active && activePatternId ? <DrillWorkspace key={active.id} drill={active} completed={knownCompletedIds.includes(active.id)} onBack={() => navigate(`/practice/${activePatternId}`)} onComplete={markComplete} /> : <Navigate to={activePatternId ? `/practice/${activePatternId}` : "/practice"} replace />} />
